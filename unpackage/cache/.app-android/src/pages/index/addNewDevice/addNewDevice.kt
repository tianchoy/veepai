        ...
        var setup: (__props: GenPagesIndexAddNewDeviceAddNewDevice) -> Any? = fun(__props): Any? {
            ...
            val handleInitDone = fun(){
                ...
                uni_scanCode(ScanCodeOptions(success = fun(res) {
                    ...
                    uni_showToast(ShowToastOptions(title = res.result, icon = "none"))
                }
                , fail = fun(err) {
                    console.error("扫描失败：", err, " at pages/index/addNewDevice/addNewDevice.uvue:67")
                }
                ))
            }
            val switchFlash = fun(){
            ...
            val scanImg = fun(){
                uni_chooseImage(ChooseImageOptions(count = 1, sourceType = _uA(
                    ...
                    uni_scanCode(ScanCodeOptions(onlyFromCamera = false, scanType = _uA(
                        ...
                        console.log("识别结果:", scanRes.result, " at pages/index/addNewDevice/addNewDevice.uvue:85")
                    }
                    , fail = fun(err){
                        console.error("识别失败:", err, " at pages/index/addNewDevice/addNewDevice.uvue:88")
                    }
                    ))
                }
                ))
