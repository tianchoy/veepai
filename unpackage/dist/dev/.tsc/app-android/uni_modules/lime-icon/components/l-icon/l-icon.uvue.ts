...
return (): any | null => {
  ...
  return isTrue(!unref(isImage) && !unref(isIconify) && !unref(isSVG))
    ? _cE("text", _uM({
        key: 0,
        class: _nC(["l-icon", [unref(classes), _ctx.lClass]]),
        style: _nS([unref(styles), _ctx.lStyle]),
        onClick: () => {_ctx.$emit('click')}
      }), _tD(unref(iconCode)), 15 /* TEXT, CLASS, STYLE, PROPS */, ["onClick"])
...
const GenUniModulesLimeIconComponentsLIconLIconStyles = [_uM([["l-icon--font", _pS(_uM([["fontFamily", "l"], ["textAlign", "center"]]))], ["@FONT-FACE", _uM([["0", _uM([["fontFamily", "l"], ["src", "url(\"/assets/t3.9658ea31.ttf\")"]])]])]])]
