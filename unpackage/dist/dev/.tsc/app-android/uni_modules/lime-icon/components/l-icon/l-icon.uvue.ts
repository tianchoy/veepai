...
return (): any | null => {
  ...
  return isTrue(!unref(isImage) && !unref(isIconify) && !unref(isSVG))
    ? createElementVNode("text", utsMapOf({
        key: 0,
        class: normalizeClass(["l-icon", [unref(classes), _ctx.lClass]]),
        style: normalizeStyle([unref(styles), _ctx.lStyle]),
        onClick: () => {_ctx.$emit('click')}
      }), toDisplayString(unref(iconCode)), 15 /* TEXT, CLASS, STYLE, PROPS */, ["onClick"])
    : isTrue((!unref(isSVG) && !unref(isIconify)) && unref(isImage))
      ? createElementVNode("image", utsMapOf({
          ...
          src: unref(iconUrl),
          onClick: () => {_ctx.$emit('click')}
        }), null, 14 /* CLASS, STYLE, PROPS */, ["src", "onClick"])
...
const GenUniModulesLimeIconComponentsLIconLIconStyles = [utsMapOf([["l-icon--font", padStyleMapOf(utsMapOf([["fontFamily", "l"], ["textAlign", "center"]]))], ["@FONT-FACE", utsMapOf([["0", utsMapOf([["fontFamily", "l"], ["src", "url(\"/assets/t3.9658ea31.ttf\")"]])]])]])]
