<template>
	<view class="j-radio" @tap.stop="wrapperClickHandler">
		<view 
		...
		<view class="j-radio--label" @tap.stop="labelClickHandler">
...
<script>
	...
	export default {
		...
		computed: {
			// 是否禁用，如果父组件j-radios-group禁用的话，将会忽略子组件的配置
			elDisabled() {
			...
			iconClasses() {
				let classes = 'j-radio--icon j-radio--icon__' + this.elShape;
				if (this.elDisabled) {
					classes += ' j-radio--icon__disabled';
				}
				if (this.isChecked && this.elDisabled) {
					classes += ' j-radio--icon__disabled__checked';
				}
				return classes
			},
			iconWrapStyle() {
		...
		methods: {
			init() {
				this.parent = $parent.call(this, 'j-radio-group');
				if (!this.parent) {
					this.usedAlone = true;
				} else { // 让父组件收集子组件
					if(typeof this.parent.radioAddChildren === 'function'){
						this.parent.radioAddChildren(this);
					}
					const obj = {
						...this.parentData
					};
				...
				// #endif
				// 设置初始化时，是否默认选中的状态，父组件j-radio-group的value可能是array，所以额外判断
				if (this.checked) {
					this.isChecked = true
				} else if (!this.usedAlone) {
					this.isChecked = value === this.name
				}else{
					// #ifdef VUE2
			...
			// 改变组件选中状态
			// 这里的改变的依据是，更改本组件的checked值为true，同时通过父组件遍历所有j-radio实例
			// 将本组件外的其他j-radio的checked都设置为false(都被取消选中状态)，因而只剩下一个为选中状态
